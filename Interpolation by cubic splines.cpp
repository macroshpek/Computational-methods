...
int n = 10000; //количество узлов интерполяции
double left = -10, right = 10; //граници интервала
...
void countСoeffs() { 
    ...
    double** arr = new double* [n]; //задаем трехдиагональную матрицу коэффициентов перед h 
    for (int i = 0; i < n; i++) {
        arr[i] = new double[n];
        for (int j = 0; j < n; j++) {
            if (i == j) {
                arr[i][j] = 4; //(потому что на равномерной сетке)
                continue;
            }
            if (abs(i - j) == 1) {
                arr[i][j] = 1;
